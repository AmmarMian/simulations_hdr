...
parser.add_argument(
    "--storage_path", required=True, help="Run directory injected by qanat."
)
parser.add_argument(
    "--name",
    default=None,
...
subplot_titles = (
    ["GLRT statistic", "Ground truth"] if n_panels == 2 else ["GLRT statistic"]
)
...
axis_common = dict(
    showgrid=False, zeroline=False, showticklabels=False, range=[0, phys_w]
)
yaxis_common = dict(
    showgrid=False,
    zeroline=False,
