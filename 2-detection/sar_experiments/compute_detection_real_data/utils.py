...
from hdrlib.core.plot_style import apply_style, EMBEDDED_STYLE_CODE
...
# Self-contained _plot.py script generated by DetectionMapExporter.
# Built in two parts so EMBEDDED_STYLE_CODE (which contains single { } braces in its
# rcParams dict) is concatenated verbatim and never passed through str.format().
# Only _DETECTION_MAP_PLOT_BODY goes through .format(); there, {{ }} produce literal
# braces and {name} are substituted.
_DETECTION_MAP_PLOT_HEADER = """\
#!/usr/bin/env python
...
_DETECTION_MAP_PLOT_BODY = """\
_mpl.rcParams.update({{"axes.grid": False}})
    ...
    def _plot_script(
        ...
        *,
        ...
        return (
            _DETECTION_MAP_PLOT_HEADER
            + EMBEDDED_STYLE_CODE
            + _DETECTION_MAP_PLOT_BODY.format(
                stem_repr=repr(stem),
                cmap_repr=repr(cmap),
