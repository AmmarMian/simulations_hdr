...
MC power curve — OnlineKroneckerGLRT vs KroneckerGLRT under H1 (change detection)
...
```sh
uv run python 2-detection/sar_experiments/mc_simulations/mc_kronecker_h1.py
```
...
| `--seed-a` | int | `0` | Seed for A1_true generation (default 0). |
| `--seed-b` | int | `1` | Seed for B1_true generation (default 1). |
| `--seed-a2` | int | `8` | Seed for A2_true (H1 distribution, default 8). |
| `--seed-b2` | int | `3` | Seed for B2_true (H1 distribution, default 3). |
| `--chunk-trials` | int | — | Split n_trials into chunks of this size for GPU batched path. Bounds per-call GPU memory: peak ∝ chunk × T_max × N × p. Default: 32 for CUDA, all trials for other backends. |
...
`2-detection/experiments/sar/sar_mc_kron_h1.yaml`
