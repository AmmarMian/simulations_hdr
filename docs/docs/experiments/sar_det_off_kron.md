...
|------|------|---------|-------------|
| `--iter-max` | int | `5` | Maximum MM iterations for Kronecker estimator (default 5). |
| `--tol` | float | `0.0001` | Convergence tolerance for MM iterations (default 1e-4). |
| `--vram` | float | — | Available VRAM in MB for GPU auto-splitting (default: auto-detect). |
