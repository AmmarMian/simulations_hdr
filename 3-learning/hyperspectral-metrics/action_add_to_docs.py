...
Reads results.npz — the ground truth and one label map per metric — and rebuilds
the row of panels as heatmaps sharing one discrete colour scale.

Output: docs/docs/assets/data/learning_hyperspectral_metrics.json
"""
...
metrics = [str(m) for m in r["metrics"]]
n_classes = int(r["n_classes"])
truth = r["truth"]

LABELS = {"euclid": "euclidien", "logeuclid": "log-euclidien",
          "riemann": "riemannien"}

scores = {}
scores_path = storage / "scores.json"
if scores_path.exists():
    scores = json.loads(scores_path.read_text()).get("scores", {})
...
for panel in panels:
    if panel == "vérité terrain":
        ...
        if entry:
            titles.append(
                f"{label} — {entry['accuracy']:.3f} / {entry['mIoU']:.3f}"
            )
        else:
            titles.append(label)
